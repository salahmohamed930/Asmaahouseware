...
const Cart: React.FC<CartProps> = ({ isOpen, onClose, items, onUpdateQuantity, onRemove, onClear, onOpenLogin }) => {
  ...
  const handleConfirmOrder = async () => {
    ...
    if (!validatePhone(customerData.phone)) {
      alert('رقم الهاتف الأساسي غير صحيح. يجب أن يبدأ بـ 01 ويتكون من 11 رقم.');
      return;
    }

    if (customerData.phone2 && !validatePhone(customerData.phone2)) {
      alert('رقم الهاتف الإضافي غير صحيح. يجب أن يبدأ بـ 01 ويتكون من 11 رقم.');
      return;
    }
            ...
            {isSuccess ? (
              ...
              <div className="space-y-5 py-2">
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="space-y-2"><label className="text-sm font-black text-gray-700">رقم الهاتف الأساسي</label><input type="tel" placeholder="01XXXXXXXXX" className="w-full p-4 bg-gray-50 border rounded-2xl focus:ring-2 focus:ring-blue-600 outline-none text-left font-bold" value={customerData.phone || ''} onChange={(e) => setCustomerData({...customerData, phone: e.target.value})} /></div>
                  <div className="space-y-2"><label className="text-sm font-black text-gray-700">رقم هاتف إضافي (اختياري)</label><input type="tel" placeholder="01XXXXXXXXX" className="w-full p-4 bg-gray-50 border rounded-2xl focus:ring-2 focus:ring-blue-600 outline-none text-left font-bold" value={customerData.phone2 || ''} onChange={(e) => setCustomerData({...customerData, phone2: e.target.value})} /></div>
                </div>
